...
class AutoFishTask_Leisure(DNAOneTimeTask, BaseDNATask):
    """AutoFishTask_Leisure
    悠闲全自动钓鱼
    """
    BAR_MIN_AREA = 1200
    ICON_MIN_AREA = 70
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.name = "自动钓鱼（悠闲）"
        self.description = "悠闲全自动钓鱼 (原作者: B站无敌大蜜瓜)"
        self.group_name = "全自动"
    ...
    def do_run(self):
        logger.info("=" * 50)
        logger.info("自动钓鱼（悠闲）启动：全自动接管 抛竿 / 授渔以鱼溜鱼 / 继续")
        logger.info("=" * 50)

        self.stats["start_time"] = time.time()
        self.info_set("当前阶段", "等待抛竿")

        last_blank_click = 0.0
        while True:
            try:
                # 1) 抛竿界面 -> 按空格开始/继续钓鱼
                if self.find_fish_cast()[0]:
                    logger.info("检测到抛竿界面 -> 按空格")
                    self.info_set("当前阶段", "已抛竿")
                    self.send_key("space", down_time=0.06)
                    self.sleep(0.5)
                    continue

                # 2) 出现鱼条（悠闲模式只有授渔以鱼需要手动溜鱼）-> 接管自动溜鱼
                (has_bar, _, _), (has_icon, _, _) = self.find_bar_and_fish_by_area()
                if has_bar or has_icon:
                    logger.info("检测到鱼条 -> 自动溜鱼")
                    self.info_set("当前阶段", "自动溜鱼")
                    self.phase_fight()
                    # 钓到鱼后点空白处关闭鱼信息层
                    self.click_relative_random(0.05, 0.3, 0.4, 0.7)
                    continue

                # 3) 其它情况：等游戏自动溜鱼 / 咬钩等；定期点空白关闭鱼信息层
                self.info_set("当前阶段", "等待")
                now = time.monotonic()
                if now - last_blank_click >= 1.0:
                    self.click_relative_random(0.05, 0.3, 0.4, 0.7)
                    last_blank_click = now
                self.sleep(0.2)
            except TaskDisabledException:
                raise
            except Exception as e:
                logger.error("AutoFishTask_Leisure fatal", e)
                break
